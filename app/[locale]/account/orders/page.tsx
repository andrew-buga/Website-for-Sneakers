...
export default function OrdersPage() {
  ...
  useEffect(() => {
    ...
    void load()
  }, [isAuthenticated, page, router, user, locale])
  ...
  return (
    <main>
      ...
      <div className="max-w-5xl mx-auto px-6 lg:px-12 py-12 min-h-screen">
        <Link href={`/${locale}/account/profile`} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors mb-8">
        ...
        <div className="space-y-4">
          {orders.map((order) => (
            <Link key={order.id} href={`/${locale}/account/orders/${order.id}`} className="block rounded-xl border p-4 hover:bg-muted/40 transition-colors">
